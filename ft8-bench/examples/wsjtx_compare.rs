...
use mfsk_core::ft8::decode::{DecodeResult, DecodeStrictness};
...
fn main() {
    ...
    let results: Vec<DecodeResult> = if use_phase2_shape {
        // `known(&[]).sic_early()` — exactly what ft8-web's decode_phase2 hits
        ...
        // one. `known`/`fft_cache` are now honoured directly by `.sic_early()`
        // (renamed from `.staged()` in mfsk-core #218), so this should match
        // plain `subtract` mode message-for-message.
    ...
    let label = if use_phase2_shape { "decode_phase2 shape (known+sic_early)" } else if use_subtract { "decode_frame_subtract (sic_early)" } else { "decode_frame" };
