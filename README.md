...
`sim_busy_band` は全信号が 16 bit に収まるよう正規化されるため 3Y0Z もデコード可能。`sim_stress_fullband` は AGC が +20 dB の crowd にロックし、-18 dB の 3Y0Z は量子化ノイズに埋没する — これが 500 Hz BPF + EQ + AP で解決する問題。BPF 単体では不十分で、フィルタ肩の歪み補正（EQ）と既知ビットのロック（AP）が組み合わさって初めてデコードに成功する。

> ※ rs-ft8n はターゲット局コール (3Y0Z) を事前指定し AP デコードを使用。WSJT-X でも DX Call フィールドに 3Y0Z を設定して AP を有効化した状態でテストしたが、デコードできなかった。差分は **適応型等化器（EQ）** — BPF 肩の振幅/位相歪みを Costas パイロットから補正する機能は rs-ft8n 独自であり WSJT-X にはない。AP なし（EQ のみ）でも -18 dB BPF edge で 30% のデコード率がある（[詳細](#bpf-edge-snr-sweep--bpf--eq--ap-の累積効果)）。
