...
| OSD 偽陽性対策 | なし | **order 別 hard_errors 閾値** (order-2: <40、order-3: <30) + score ≥ 2.5 ゲート |
...
## WASM デモ — ブラウザで WSJT-X を超える

**[https://jl1nie.github.io/rs-ft8n/](https://jl1nie.github.io/rs-ft8n/)** を開き、以下の WAV ファイルをドロップするだけで FT8 デコードを体験できる。
...
3. WAV をドラッグ＆ドロップ → デコード結果とタイミングが表示される
4. 「Multi-pass subtract」にチェックを入れると 3 パス逐次干渉除去が有効になる

### WSJT-X との比較方法

同じ WAV ファイルを WSJT-X にロードして結果を比較できる:

1. WSJT-X を起動 → File → Open → テスト WAV を選択
2. Mode: FT8、Decode ボタンで実行
...
| `sim_stress_fullband.wav` | crowd +20 dB / target -18 dB / **AGC が crowd にロック → ADC 飽和** | 10 局（3Y0Z なし） | **15 局（3Y0Z なし）** |
| **`sim_stress_bpf_edge_clean.wav`** | target -18 dB / BPF edge -3 dB / target + noise のみ | **デコード不能** | **CQ 3Y0Z JD34 (197 ms)** ※ |

`sim_busy_band` は全信号が 16 bit に収まるよう正規化されるため 3Y0Z もデコード可能。`sim_stress_fullband` は AGC が +20 dB の crowd にロックし、-18 dB の 3Y0Z は量子化ノイズに埋没する。`sim_stress_bpf_edge_clean` は BPF 通過後の target + noise のみの音声で、BPF 肩の -3 dB 減衰と位相歪みが残る。

> rs-ft8n はターゲット局コール (3Y0Z) を事前指定した AP デコードと、BPF 肩歪みを補正する適応型イコライザ (EQ) を併用している。WSJT-X でも DX Call に 3Y0Z を設定して AP を有効化したがデコードには至らなかった。EQ による BPF 肩補正が寄与していると考えられる。AP なし (EQ のみ) でも -18 dB BPF edge で 30% のデコード率がある ([詳細](#bpf-edge-snr-sweep--bpf--eq--ap-の累積効果))。

> **Multi-pass subtract** にチェックを入れると、強信号をデコード→波形減算→残余から弱信号を再探索する 3 パス逐次干渉除去が有効になる。チェックなし（single-pass）より多くの局をデコードできるが、処理時間は長くなる。
...
| `sim_stress_bpf_edge_clean.wav` | target @ -18 dB、BPF edge -3 dB（WSJT-X 不能） |
| `sim_stress_bpf_edge.wav` | 上の混合版（crowd 漏洩あり） |
| `sim_bpf_center.wav` | BPF center 配置 |
| `sim_bpf_shoulder.wav` | BPF shoulder 配置 |
| `sim_bpf_edge.wav` | BPF edge 配置 |
| `sim_bpf_subtract.wav` | BPF 帯域内 crowd + subtract |
| `sim_busy_band_hard_mixed.wav` | crowd +40 dB、AGC clipping |
| `sim_busy_band_hard_bpf.wav` | 上の BPF 後 |
| `sim_interference.wav` | +40 dB 隣接妨害 |
...
PCM 16-bit 12 kHz
  ...
  ├→ OSD fallback (order-2: pass=4、order-3: pass=5、sync_q ≥ 12 時)
...
OSD は多数の候補を探索するため、ノイズが偶然 CRC-14 (1/16384) を通過する確率が無視できない。order が高いほど候補数が多く偽陽性リスクが上がるため、hard_errors 閾値を order 別に設定:

| OSD order | 候補数 | CRC 衝突期待値 | hard_errors 上限 |
|-----------|--------|---------------|-----------------|
| order-2 (pass=4) | ~4,187 | 26% | < 40 |
| order-3 (pass=5) | ~121,667 | 7.4 回 | < 30 |
