...
`sim_busy_band` は全信号が 16 bit に収まるよう正規化されるため 3Y0Z もデコード可能。`sim_stress_fullband` は AGC が +20 dB の crowd にロックし、-18 dB の 3Y0Z は量子化ノイズに埋没する — これが 500 Hz BPF で解決する ADC ダイナミックレンジ問題。

> ※ WASM デモの subtract モードは EQ + AP（ターゲット局コール 3Y0Z を事前指定）を使用。WSJT-X は単独 WAV では QSO コンテキストがないため AP が効かない。条件を揃えた AP なし（EQ のみ）でも -18 dB BPF edge で 30% のデコード率がある（[詳細](#bpf-edge-snr-sweep--bpf--eq--ap-の累積効果)）。
